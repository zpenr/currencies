````markdown
# 💱 Currency Exchange API

REST API для работы с валютами и обменными курсами.

🌐 **Базовый URL**: `http://91.200.84.196:8000`

---

## 🔐 Аутентификация

Все запросы требуют заголовок:
```
X-API-KEY: <ваш_ключ>
```

---

## 📚 Endpoints

### 💵 Валюты

#### `GET /currencies`
Получить список всех валют.

**Ответ (200):**
```json
[
  {
    "id": 0,
    "name": "United States dollar",
    "code": "USD",
    "sign": "$"
  }
]
```

#### `GET /currency/{code}`
Получить валюту по 3-буквенному коду.

**Пример:** `GET /currency/USD`

**Ответ (200):**
```json
{
  "id": 0,
  "name": "United States dollar",
  "code": "USD",
  "sign": "$"
}
```

**Ошибки:**
- `404` — Валюта не найдена
- `400` — Некорректный формат кода

#### `POST /currencies`
Добавить новую валюту.

**Content-Type:** `application/x-www-form-urlencoded`

**Параметры:**
| Параметр | Обязательный | Описание |
|----------|-------------|----------|
| `name` | ✅ | Полное название валюты |
| `code` | ✅ | 3-буквенный код (уникальный) |
| `sign` | ✅ | Символ валюты |

**Пример запроса:**
```bash
curl -X POST http://91.200.84.196:8000/currencies \
  -H "X-API-KEY: ваш_ключ" \
  -d "name=Russian Ruble&code=RUB&sign=₽"
```

**Ответ (201):** Объект созданной валюты

**Ошибки:**
- `400` — Отсутствует нужное поле формы
- `409` — Валюта с таким кодом уже существует

#### `DELETE /currency/{code}`
Удалить валюту по коду.

**Пример:** `DELETE /currency/USD`

**Ответ (200):**
```json
{ "status": "succes" }
```

---

### 🔄 Обменные курсы

#### `GET /exchangeRates`
Получить все обменные курсы.

**Ответ (200):**
```json
[
  {
    "id": 1,
    "baseCurrency": { "id": 0, "name": "United States dollar", "code": "USD", "sign": "$" },
    "targetCurrency": { "id": 1, "name": "Euro", "code": "EUR", "sign": "€" },
    "rate": 0.954201
  }
]
```

#### `GET /exchangeRate/{baseCode}{targetCode}`
Получить курс для пары валют (6 букв: 3 + 3).

**Пример:** `GET /exchangeRate/USDEUR`

**Ответ (200):**
```json
{
  "id": 1,
  "baseCurrency": { "id": 0, "name": "United States dollar", "code": "USD", "sign": "$" },
  "targetCurrency": { "id": 1, "name": "Euro", "code": "EUR", "sign": "€" },
  "rate": 0.954201
}
```

**Ошибки:**
- `404` — Обменный курс для пары не найден
- `400` — Некорректный формат адресов

#### `POST /exchangeRates`
Добавить новый обменный курс.

**Content-Type:** `application/x-www-form-urlencoded`

**Параметры:**
| Параметр | Обязательный | Описание |
|----------|-------------|----------|
| `baseCurrencyCode` | ✅ | Код базовой валюты |
| `targetCurrencyCode` | ✅ | Код целевой валюты |
| `rate` | ✅ | Курс обмена (десятичное число) |

**Пример:**
```bash
curl -X POST http://91.200.84.196:8000/exchangeRates \
  -H "X-API-KEY: ваш_ключ" \
  -d "baseCurrencyCode=USD&targetCurrencyCode=RUB&rate=92.50"
```

**Ответ (201):** Объект созданного курса

**Ошибки:**
- `400` — Отсутствует нужное поле формы
- `404` — Одна из валют не существует в БД
- `409` — Валютная пара уже существует

#### `PATCH /exchangeRate/{baseCode}{targetCode}`
Обновить курс для существующей пары.

**Content-Type:** `application/x-www-form-urlencoded`

**Параметры:**
| Параметр | Обязательный | Описание |
|----------|-------------|----------|
| `rate` | ✅ | Новый курс обмена |

**Пример:**
```bash
curl -X PATCH http://91.200.84.196:8000/exchangeRate/USDEUR \
  -H "X-API-KEY: ваш_ключ" \
  -d "rate=0.960000"
```

**Ответ (200):** Обновлённый объект курса

**Ошибки:**
- `400` — Отсутствует поле `rate`
- `404` — Валютная пара не найдена

---

### 💱 Конвертация

#### `GET /exchange?from={code}&to={code}&amount={value}`
Конвертировать сумму из одной валюты в другую.

**Параметры запроса:**
| Параметр | Обязательный | Описание |
|----------|-------------|----------|
| `from` | ✅ | Код исходной валюты |
| `to` | ✅ | Код целевой валюты |
| `amount` | ✅ | Сумма для конвертации |

**Пример:** `GET /exchange?from=USD&to=EUR&amount=100`

**Ответ (200):**
```json
{
  "baseCurrency": { "id": 0, "name": "United States dollar", "code": "USD", "sign": "$" },
  "targetCurrency": { "id": 1, "name": "Euro", "code": "EUR", "sign": "€" },
  "rate": 0.954201,
  "amount": 100.0,
  "convertedAmount": 95.42
}
```

**Логика конвертации:**
1. Прямой курс `from → to`
2. Обратный курс `to → from` (инвертируется)
3. Кросс-курс через USD, если доступны оба курса к доллару
4. `null`, если конвертация невозможна

---

## ⚠️ Коды ответов

| Код | Описание |
|-----|----------|
| `200` | Успешный запрос (GET, PATCH, DELETE) |
| `201` | Ресурс успешно создан (POST) |
| `400` | BadRequest — некорректные параметры запроса |
| `401` | Invalid or missing API Key |
| `404` | NotFound — ресурс не найден |
| `409` | Conflict — конфликт состояния (дубликат) |
| `500` | Server Error — внутренняя ошибка сервера |

---

## 🗄️ База данных

- **Тип:** SQLite (`currencies.db`)
- **Таблицы:**
  - `currencies` — id, name, code (UNIQUE), sign
  - `ExchangeRates` — ID, BaseCurrencyId, TargetCurrencyId, Rate (UNIQUE пара)

При запуске сервера автоматически создаются таблицы и добавляются базовые валюты (USD, EUR, JPY, GBP, CNY, AUD, CAD, CHF, HKD, SGD) и начальные курсы.

---

## 🚀 Запуск сервера

```bash
python main.py
```

Сервер запустится на `0.0.0.0:8000`.

---

---

## 🧪 Примеры запросов cURL

```bash
# Получить все валюты
curl http://91.200.84.196:8000/currencies -H "X-API-KEY: ваш_ключ"

# Получить курс USD → EUR
curl http://91.200.84.196:8000/exchangeRate/USDEUR -H "X-API-KEY: ваш_ключ"

# Конвертировать 100 USD в EUR
curl "http://91.200.84.196:8000/exchange?from=USD&to=EUR&amount=100" -H "X-API-KEY: ваш_ключ"

# Добавить новую валюту
curl -X POST http://91.200.84.196:8000/currencies \
  -H "X-API-KEY: ваш_ключ" \
  -d "name=Russian Ruble&code=RUB&sign=₽"

# Обновить курс
curl -X PATCH http://91.200.84.196:8000/exchangeRate/USDEUR \
  -H "X-API-KEY: ваш_ключ" \
  -d "rate=0.960000"
```

---

> 💡 **Примечание:** Все строковые коды валют регистронезависимы на входе, но возвращаются в верхнем регистре.

Разработано на Python 3 с использованием стандартной библиотеки `http.server` и `sqlite3`.
````